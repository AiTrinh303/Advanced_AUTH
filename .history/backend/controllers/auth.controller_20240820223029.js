...
//8. CHECK AUTH AND ADMIN - get admin data
//- Find the user with the userId
//- If user not found, return an error
//- If user is not an admin, return an error
//- If user is an admin, send the user data in response
//- Handle errors

export const checkAuthAndAdmin = async (req, res) => {
	try {
		const user = await User.findById(req.userId).select("-password");
		if (!user) {
			return res.status(400).json({ success: false, message: "User not found" });
		...
		console.log("Error in checkAuthAndAdmin ", error);
		res.status(400).json({ success: false, message: error.message });
	}
};
