...
export const useAuthStore = create((set) => ({
	...
	updateUser: async (email, password, name) => {
		set({ isLoading: true, error: null });
		try {
			const response = await axios.put(`http://localhost:5000/user/update/${user.id}`, { email, password, name });
			set({ user: response.data.user, isAuthenticated: true, isLoading: false });
		} 
		catch (error) {
			set({ error: error.response.data.message || "Error update user", isLoading: false });
			throw error;
		}
	},
