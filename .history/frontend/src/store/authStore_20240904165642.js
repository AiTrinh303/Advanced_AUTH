...
const API_URL_2 = import.meta.env.MODE === "development" ? "http://localhost:5000/user" : "/user";
...
export const useAuthStore = create((set) => ({
	...
	updateUser: async (userData) => {
        ...
        try {
            const response = await axios.put(`http://localhost:5000/user/update/${userData.id}`, userData);
	...
	checkAuth: async () => {
        ...
        try {
            ...
            set({ user: response.data.user ? (response.data.user._doc || response.data.user) : {}, isAuthenticated: true, isCheckingAuth: false });
        } catch (error) {
            set({ error: null, isCheckingAuth: false, isAuthenticated: false, user: {} });
        }
    },
	...
	resetPassword: async (token, password) => {
		...
		try {
			...
			set({ message: response.data.message, isLoading: false, isAuthenticated: true });
		} catch (error) {
			set({
				isLoading: false,
